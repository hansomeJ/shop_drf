        ...
        success: function (res) {
            ...
            for (var i = 0; i < res.results.length; i++) {
                $("#view").append(
                    ...
                    '</div>' +
                    '<div class="ui-block-b" id="b' + res.results[i].id + '" style="padding-top:50px;">' + "价格：" +
                    res.results[i].price +
                    '<br><br>' + "名称：" +
                    res.results[i].model +
                    '</div>'
...
$(document).on("pageshow", "#productDetails", function () {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            $("#price").html("价格：" + res.price);
            $("#model").html("名称：" + res.model);
            $("#description").html("简介：" + res.description);
            $("#manufacturer").html("品牌：" + res.manufacturer.name);
...
$(document).on("pageshow", "#notice", function () {
    ...
    $.ajax({
        url: "http://127.0.0.1:8001/computer/notice/",
        headers: myheader(),
        success: function (res) {
            // console.log(res);
        ...
        success: function (res) {
            ...
            for (var i = 0; i < res.results.length; i++) {
                ...
                $("#view").append(
                    ...
                    '</div>' +
                    '<div class="ui-block-b" style="padding-top:20px;">' +
                    '物资：' + res.results[i].product.model + '<br>' +
                    '状态：' + status_text + '<br>' +
                    ...
                    // '</div>'+
                    '<div class="ui-block-b">' +
                    '<div class="gotoDetails2" style="text-align:center;" value="' + res.results[i].id + '"><button class="ui-btn ui-btn-inline">归还</button></div>' +
                    '</div>'
                );
